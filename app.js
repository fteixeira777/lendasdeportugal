...
const allQuests=[...ORIGINAL_QUESTS.map((q,i)=>({...q,...extra[i],region:'lisbon',radius:40})),...PACOS_QUESTS];
const regions={lisbon:{name:'Lisboa',start:{lat:38.7117,lng:-9.1385}},pacos:{name:'Paços de Ferreira',start:{lat:41.2775,lng:-8.3744}}};
let region='pacos',quests=allQuests.filter(q=>q.region===region);
...
function read(){try{state=R.restore(localStorage.getItem(key()),allQuests);}catch{storageOK=false;state={found:[]};}}
...
function mapPoint(p){if(region==='pacos')return {x:500+(p.lng+8.3749)*111320*Math.cos(41.276*Math.PI/180)*.65,y:290-(p.lat-41.276)*111320*.65};return {x:120+(p.lng+9.1400)/.012*720,y:70+(38.715-p.lat)/.009*430};}
function renderMap(){
 ...
 if(position){const p=mapPoint(position),on=p.x>=20&&p.x<=980&&p.y>=20&&p.y<=560;$('player').style.display=on?'':'none';$('player').setAttribute('transform',`translate(${p.x},${p.y}) rotate(${heading||0})`);text('map-help',!on?'You are outside the illustrated '+regions[region].name+' area':mode==='demo'?'Demo · arrow keys or “Walk toward relic”':'Live GPS · north-up illustration');}else{$('player').style.display='none';text('map-help',mode==='live'?'Waiting for your location…':'Choose demo or live GPS to begin');}
}
function render(){
 const q=quests[active],total=R.tokens(state,allQuests),done=state.found.includes(q.id),count=quests.filter(q=>state.found.includes(q.id)).length,all=count===quests.length;
 text('region-heading','PORTUGAL / '+regions[region].name.toUpperCase());text('map-region',regions[region].name.toUpperCase()+' · PILOT ROUTE');text('collection-title',regions[region].name);text('collection-desc',quests.length+' relics · '+regions[region].name);text('quest-count',String(quests.length).padStart(2,'0')+' RELICS TO DISCOVER');
 $('lisbon-art').style.display=region==='lisbon'?'':'none';$('pacos-art').style.display=region==='pacos'?'':'none';$('map-svg').setAttribute('aria-label','Illustrated '+regions[region].name+' quest map, north up.');
 $('quest-list').style.setProperty('--quest-columns',quests.length);
 text('quest-place',q.place);
 text('mode-label',mode==='demo'?'Demo adventure':mode==='live'?'Live GPS':'Choose a mode');
 text('journal-count',`${state.found.length}/${allQuests.length}`);text('nav-points',total);text('reward-total',total);
 $('progress').style.width=(count/quests.length*100)+'%';text('progress-label',all?'Collection complete!':`${count} of ${quests.length} relics discovered`);
 ...
 text('active-symbol',q.symbol);text('quest-name',all?'The old city knows your name.':q.name);text('quest-desc',all?'All relics in this city are in your journal. Choose another city to keep exploring.':q.desc.replace(/^"|"$/g,''));text('quest-kicker',done?'RELIC DISCOVERED':ready()?'YOU ARE IN DISCOVERY RANGE':'YOUR NEXT DISCOVERY');text('distance',distanceText(q)+' away');text('quest-reward','✧ '+q.tokens+' explorer points');
 ...
 $('journal-list').innerHTML=allQuests.map((q,i)=>{const found=state.found.includes(q.id);return `<article class="journal-card ${found?'':'locked'}"><span class="eyebrow">${found?'DISCOVERED · +'+q.tokens+' POINTS':'UNDISCOVERED · CHAPTER 0'+(i+1)}</span><h3>${found?q.symbol:'◇'} ${q.name}</h3><p>${found?q.lore:'A story still waiting to be found. Follow the clue and solve its mystery to add this relic to your journal.'}</p><button class="text-btn" data-follow="${q.id}">${found?'Revisit on map':'Follow this mystery'} →</button></article>`;}).join('');
 $('badge').innerHTML=Object.entries(regions).map(([id,r])=>{const group=allQuests.filter(q=>q.region===id),n=group.filter(q=>state.found.includes(q.id)).length;return `<div class="city-badge ${n===group.length?'earned':''}">♜ ${r.name} <span>${n===group.length?'UNLOCKED ✓':n+' / '+group.length+' relics'}</span></div>`;}).join('');
...
function selectRegion(id){if(!regions[id])return;stopWalk();closeCamera();region=id;quests=allQuests.filter(q=>q.region===id);start=regions[id].start;active=Math.max(0,quests.findIndex(q=>!state.found.includes(q.id)));$('region-select').value=id;if(mode==='demo')position={...start,accuracy:0,time:Date.now()};render();}
...
$('journal-list').onclick=e=>{const b=e.target.closest('[data-follow]');if(b){const q=allQuests.find(q=>q.id===b.dataset.follow);selectRegion(q.region);active=quests.findIndex(item=>item.id===q.id);changeView('explore');render();}};
